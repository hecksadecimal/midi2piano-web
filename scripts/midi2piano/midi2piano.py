...
def obtain_midi_from_stdin():
    """
    An alternative input method to take MIDI data from STDIN.
    """
    fileno = sys.stdin.fileno()
    file = open(fileno, "rb", closefd=False).read()
    return file
...
def script_run():
    ...
    Used for automation.
    """
    midi_file = obtain_midi_from_stdin()
    if not midi_file:
        return # Cancel
    score = midi2score_without_ticks(midi_file)
    score = filter_events_from_score(score)
